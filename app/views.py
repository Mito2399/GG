...
#
# REVISED ROLE MODEL
#   Admin (Manager)    — full access + employee management
#   Accounting Staff   — same permissions as Admin except employee mgmt:
#                        add/view/edit clients, print financial reports,
#                        process payments, manage bookings
#   Marketing Staff    — manage bookings only
#   General Staff      — view-only access (records/lots/bookings) for
#                        inventory purposes — no write permissions anywhere
#
...
# ─────────────────────────────────────────────── clients ──────────────────────
# Add / Edit / Delete client — Admin + Accounting Staff only.

@login_required(login_url="login")
def add_client_view(request):
    if not _check_write_role(request, "Accounting Staff"):
        messages.error(request, "Access restricted. Only Accounting Staff can add new clients.")
...
def _filter_client_records(request):
    """
    Shared filter logic used by BOTH records_view (screen) and
    export_records_excel (Export). Same pattern as _filter_lots() —
    guarantees the export always matches whatever was searched on screen.

    Returns: (filtered_queryset, query_string, active_filters_dict)
    """
    ...
    active_filters = {"Search": query} if query else {}

    return clients, query, active_filters


@login_required(login_url="login")
def records_view(request):
    clients, query, active_filters = _filter_client_records(request)
...
def edit_details_view(request, pk):
    ...
    if request.method == "POST":
        if not _check_write_role(request, "Accounting Staff"):
            messages.error(request, "Access restricted. Only Accounting Staff can edit client records.")
...
# ─────────────────────────────────────────────── payments ─────────────────────
# View & process payments — Admin + Accounting Staff only.
...
# ─────────────────────────────────────────────── plan ─────────────────────────
# Assign / cancel plans — Admin + Accounting Staff only.
...
def plan(request, pk):
    ...
    if request.method == "POST":
        if not _check_write_role(request, "Accounting Staff"):
            messages.error(request, "Access restricted. Only Accounting Staff can assign plans.")
...
# ─────────────────────────────────────────────── lots ─────────────────────────
# View-only for everyone (General Staff uses this for inventory purposes).

def _filter_lots(request):
    """
    Shared filter logic used by BOTH lots_view (screen) and export_lots_excel
    (Export/Print). This guarantees the exported/printed data always matches
    whatever filters are currently applied on screen — no more "export
    dumps everything" mismatch.

    Returns: (filtered_queryset, query_context_dict, active_filters_dict)
    """
    lots = (
        ClientStatus.objects
        .exclude(plan="No Plan")
    ...
    # Human-readable summary of active filters (for export/print header)
    active_filters = {
        "Lot Type":     q_type,
        "Phase":        q_phase,
        "Block":        q_block,
        "Section":      q_section,
        "Lot No.":      q_lot,
        "Column Level": q_column_level,
        "Col. Type":    q_columbarium_type,
        "Col. Level":   q_columbarium_level,
        "Tomb No.":     q_tomb_number,
        "Status":       q_status,
    }
    active_filters = {k: v for k, v in active_filters.items() if v}

    return lots, query_context, active_filters


@login_required(login_url="login")
def lots_view(request):
    lots, query_context, active_filters = _filter_lots(request)
    ...
    return render(request, "app/lots.html", {
        "lots":                 lots,
        "active_filters":       active_filters,
        "q_type":               query_context["q_type"],
        "q_phase":              query_context["q_phase"],
        "q_block":              query_context["q_block"],
        "q_section":            query_context["q_section"],
        "q_lot":                query_context["q_lot"],
        "q_status":             query_context["q_status"],
        "q_column_level":       query_context["q_column_level"],
        "q_columbarium_type":   query_context["q_columbarium_type"],
        "q_columbarium_level":  query_context["q_columbarium_level"],
        "q_tomb_number":        query_context["q_tomb_number"],
        "plan_choices":         plan_choices,
    })

# ─────────────────────────────────────────────── bookings ─────────────────────
# Manage bookings — Admin + Accounting Staff + Marketing Staff.
# General Staff has view-only access (page renders for everyone; writes are gated).
...
def bookings_view(request):
    ...
    if request.method == "POST":
        ...
        # ── Mark as No Show ───────────────────────────────────────────────────
        if action == "no_show":
            if not _check_write_role(request, "Accounting Staff", "Marketing Staff"):
                return JsonResponse({"success": False, "error": "Access restricted."})
            if not _check_pin(request.user, request.POST.get("pin", "")):
                return JsonResponse({"success": False, "error": "Invalid PIN."})
        ...
        # ── Add new booking ───────────────────────────────────────────────────
        if not _check_write_role(request, "Accounting Staff", "Marketing Staff"):
            messages.error(request, "Access restricted. Only Accounting or Marketing Staff can add bookings.")
...
def export_lots_excel(request):
    """
    Exports the Lots list to Excel.
    FIX: now reuses _filter_lots() so the export always matches whatever
    filters (lot type, phase, block, section, status, etc.) are currently
    applied on the Lots screen — it no longer dumps the entire table
    regardless of what was searched.
    """
    import datetime
    today = datetime.date.today().strftime("%B %d, %Y")

    lots, _query_context, active_filters = _filter_lots(request)
    ...
    # Show which filters were active at export time, for traceability.
    if active_filters:
        filter_summary = " | ".join(f"{label}: {value}" for label, value in active_filters.items())
        ws.append([f"Filters applied: {filter_summary}"])
        ws.cell(row=ws.max_row, column=1).font = Font(italic=True, size=10, color="666666")
    else:
        ws.append(["Filters applied: None (showing all lots)"])
        ws.cell(row=ws.max_row, column=1).font = Font(italic=True, size=10, color="666666")

    ws.append([f"Total records: {lots.count()}"])
    ws.cell(row=ws.max_row, column=1).font = Font(italic=True, size=10, color="666666")
    ws.append([])
    ...
    ws.freeze_panes = "A7"   # header row now sits 2 rows lower (filter summary + total count)
    _xl_auto_width(ws)

    suffix = "filtered" if active_filters else "all"
    return _xl_response(wb, f"lots_{suffix}_{datetime.date.today()}.xlsx")
...
def export_records_excel(request):
    """
    Exports Client Records to Excel.
    FIX: now reuses _filter_client_records() so the export matches whatever
    was typed into the search box on screen — it no longer dumps every
    client regardless of what was searched.
    """
    import datetime
    today = datetime.date.today().strftime("%B %d, %Y")
    ...
    # Show what was searched at export time, for traceability.
    if active_filters:
        ws.append([f"Search applied: \"{query}\""])
    else:
        ws.append(["Search applied: None (showing all clients)"])
    ws.cell(row=ws.max_row, column=1).font = Font(italic=True, size=10, color="666666")

    ws.append([f"Total records: {clients.count()}"])
    ws.cell(row=ws.max_row, column=1).font = Font(italic=True, size=10, color="666666")
    ws.append([])
    ...
    ws.freeze_panes = "A7"   # header row now sits 2 rows lower (search summary + total count)
    _xl_auto_width(ws)

    suffix = "filtered" if active_filters else "all"
    return _xl_response(wb, f"client_records_{suffix}_{datetime.date.today()}.xlsx")

...
# FIX: shared PIN validator — accepts a raw 4-digit string, preserving leading zeros.
# Returns the digit string (not an integer) so "0010" stays "0010".
...
class EmployeeCreateForm(BootstrapForm):
    ...
    # FIX: explicit CharField so leading zeros (e.g. "0010") are preserved.
    # The old ModelForm mapping produced IntegerField which silently dropped them.
    pin = forms.CharField(
        max_length=4,
    ...
    # FIX: use shared validator; returns digit string, not int
    def clean_pin(self):
...
class EmployeeUpdateForm(forms.Form):
    ...
    # FIX: CharField to preserve leading zeros
    pin = forms.CharField(
        max_length=4,
        widget=forms.TextInput(attrs={
            "inputmode": "numeric",
...
class PlanForm(forms.Form):
    # "No Plan" excluded — staff should only assign real plans via this form.
    ...
    block = forms.CharField(
        max_length=200, required=False,
        widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "e.g. B-1"}),
    )
...
class BookingForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        # FIX: validate uniqueness in the form so the user gets a friendly error
        # instead of a raw IntegrityError from the DB unique_together constraint.

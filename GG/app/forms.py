...
class PlanForm(forms.Form):
    ...
    contract_number = forms.CharField(
        max_length=200, required=False,
        widget=forms.TextInput(attrs={
            "class": "form-control", "placeholder": "Contract / P.A. number"
        }),
    )
    ...
    tomb_number = forms.CharField(
        max_length=200, required=False,
        widget=forms.TextInput(attrs={
            "class": "form-control", "placeholder": "Tomb Number",
        }),
        label="Tomb Number",
    )
...
class BookingForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        # Only Active bookings block a slot — cancelled bookings free it up.
        if booking_date and booking_time:
            qs = Booking.objects.filter(
                booking_date=booking_date,
                booking_time=booking_time,
                status="Active",          # ← FIX: ignore cancelled bookings
            )
            if self.instance.pk:

...
class ClientPersonalInfo(models.Model):
    ...
    # FIX: full_name is now a @property so it works correctly in both
    # Python code (client.full_name) and Django templates ({{ client.full_name }}).
    @property
    def full_name(self):
...
class ClientStatus(models.Model):
    # FIX: "No Plan" added as a valid choice so the placeholder row
    # created on client registration does not violate field constraints.
    PLAN_CHOICES = [
        ("No Plan",         "No Plan"),          # internal placeholder only
        ("Lawn lot",        "Lawn lot"),
    ...
    # FIX: removed stray max_length kwarg that IntegerField ignores
...
class Booking(models.Model):
    ...
    class Meta:
        ...
        # FIX: prevent double-booking the same date+time slot at the DB level
...
class UserLog(models.Model):
    ...
    # FIX: PIN stored as a bcrypt/PBKDF2 hash (via Django's make_password).
    # CharField(max_length=128) holds the full hash string.
    # Leading zeros are now preserved because the raw digit string is hashed,
    # not cast to an integer. Use check_password(entered, stored) to verify.
    pin = models.CharField(max_length=128)

    # FIX: full_name as @property — consistent with ClientPersonalInfo
    @property
    def full_name(self):

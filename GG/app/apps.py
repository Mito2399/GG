...
def _daily_backup_loop(hour: int = 18, minute: int = 0) -> None:
    ...
    logger.info(
        "[Scheduler] Daily backup scheduler active — fires every day at %02d:%02d (%s)",
        hour, minute, getattr(django_timezone.get_current_timezone(), "key",
                              str(django_timezone.get_current_timezone())),
    )

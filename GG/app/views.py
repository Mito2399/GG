...
# FIX: replaced _get_pin() (which returned the raw stored value for string
# comparison) with _check_pin() which uses Django's check_password() for
# hashed employee PINs while doing a plain string comparison for the admin
# PIN read from settings.  This also fixes the IntegerField leading-zero bug:
# employee PIN "0010" was stored as integer 10, so str(log.pin) == "10" never
# matched the entered "0010".  With hashed storage the raw digit string is
# hashed and check_password() handles the comparison correctly.
...
# FIX: _admin_required is now the single decorator needed for admin-only views.
# Previously @login_required was also stacked on top, which was redundant
# because _admin_required already checks is_authenticated.  Keeping both is
# harmless but fragile (order-dependent).  We embed the authentication check
# inside _admin_required so callers only need one decorator.
...
def _log_activity(user, action: str):
    """Write an activity string to the employee's UserLog row (best-effort)."""
...
def homepage_view(request):
    # FIX: use aggregate(Sum()) instead of Python sum() over a queryset —
    # more efficient and returns a proper Decimal rather than a float.
    total_clients = ClientPersonalInfo.objects.count()

    # FIX: exclude "No Plan" placeholder rows from the active-plan count
    active_status = (
    ...
    # FIX: pending_payments = individual unpaid payment rows, not the sum of
    # months_remaining across all clients (which was a meaningless aggregate).
    pending_payments = (
        Payment.objects
    ...
    # FIX: real recent-activity feed instead of hardcoded placeholder data.
    recent_payments = (
        Payment.objects
        .filter(is_paid=True)
        .select_related("client_status__client")
        .order_by("-date_paid")[:5]
    )
    recent_logs = (
        UserLog.objects
        .filter(time_in__isnull=False)
        .exclude(activities__in=["Account created", "", None])
        .order_by("-time_in")[:5]
    )
...
def add_client_view(request):
    if request.method == "POST":
        ...
        if form.is_valid() and beneficiary_formset.is_valid():
            ...
            # Log activity
            _log_activity(request.user, "Add Client")
...
def edit_details_view(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and beneficiary_formset.is_valid():
            ...
            # FIX: log edit activity (was missing)
            _log_activity(request.user, "Edit Record")
...
# FIX: added @login_required — previously missing, allowing unauthenticated
# requests to reach this endpoint (request.user would be AnonymousUser and
# _check_pin would crash or silently fail).
@login_required(login_url="login")
def delete_client_view(request, pk):
    ...
    # FIX: log delete activity
    _log_activity(request.user, "Delete Client")
...
def payment_history_view(request, pk):
    ...
    # FIX: also redirect when the client only has a "No Plan" placeholder row,
    # which previously rendered a confusing "0 months total" payment table.
    ...
    if request.method == "POST":
        ...
        # FIX: use _check_pin (hashed comparison) instead of string comparison
...
def plan(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # FIX: consistent blocking logic — only block when a real
            # (non-placeholder) plan already exists, regardless of status.
            # The snippet file used status__in=[True, False] which was more
            # correct than the production code that blocked on .exists() alone.
            ...
            # Log activity
            _log_activity(request.user, "Add Plan")
...
def lots_view(request):
    ...
    # Exclude "No Plan" from the filter dropdown
    plan_choices = [
        c[0] for c in ClientStatus.PLAN_CHOICES if c[0] != "No Plan"
    ]
...
def calendar_view(request):
    ...
    return render(request, "app/calendar.html", {
        ...
        # FIX: pass day-name list from Python so the template doesn't need
        # the non-existent built-in |split filter (which caused TemplateSyntaxError).
...
# FIX: removed redundant @login_required — _admin_required already checks
# is_authenticated before the admin check, so stacking both was fragile.
@_admin_required
        ...
        if form.is_valid():
            ...
            with transaction.atomic():
                ...
                UserLog.objects.create(
                    ...
                    # FIX: store a hashed PIN so plaintext is never in the DB.
                    # d["pin"] is a validated 4-digit digit string (e.g. "0010").
...
def edit_employee_view(request, pk):
    ...
    if request.method == "POST":
        ...
        # NOTE: we don't pre-fill the PIN field because the stored value is a
        # hash — the admin must enter a new 4-digit PIN when editing.
        form = EmployeeUpdateForm(initial={
            ...
            # pin intentionally left blank — admin sets a new one
        })

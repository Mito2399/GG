...
def cancel_plan_view(request, pk):
    ...
    _log_activity(request.user, "Cancel Plan")
...
def bookings_view(request):
    form     = BookingForm()
    # Show all bookings (Active + Cancelled) — template handles visual distinction
...
def calendar_view(request):
    ...
    # FIX: only count Active bookings as "booked" dates
    ...
    if selected_date_str:
        try:
            ...
            # FIX: only Active bookings block a time slot
            booked_times  = set(
